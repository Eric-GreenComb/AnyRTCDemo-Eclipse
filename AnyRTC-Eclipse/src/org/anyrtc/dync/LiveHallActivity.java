...
public class LiveHallActivity extends Activity {
    ...
    public void OnBtnClicked(View btn) {
        ...
        if (btn.getId() == R.id.btn_back) {
            finish();
        } else if (btn.getId() == R.id.btn_customer_service_live) {
            Intent i = new Intent(this, LiveGuestActivity.class);
            i.putExtra("AnyrtcID", "800000000028");//这里对应着你所申请的AnyRTC ID
            i.putExtra("UserName", "Eric@Mao" + (int) (1 + Math.random() * 100));//1-100随机数
            this.startActivity(i);
        } else if (btn.getId() == R.id.btn_live) {
            Intent i = new Intent(this, LiveHostActivity.class);
